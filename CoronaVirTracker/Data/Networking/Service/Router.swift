...
class Router<EndPoint: EndPointType>: NetworkRouter {
    ...
    var timeoutInterval: TimeInterval = 34
    ...
    func request(_ route: EndPoint, completion: @escaping NetworkRouterCompletion) {
        _ = requestWithReturn(route, completion: completion)
    }
...
//            print(request.allHTTPHeaderFields)
    ...
    fileprivate func addAdditionalHeaders(_ additionalHeaders: HTTPHeaders?, request: inout URLRequest) {
        var headers = additionalHeaders ?? HTTPHeaders()
        if let lCode = Locale.current.languageCode, headers["Accept-Language"] == nil  {
            headers.updateValue(lCode, forKey: "Accept-Language")
        }
        
        if let token = KeychainUtility.getCurrentUserToken(), headers["Authorization"] == nil {
            headers.updateValue("Token \(token)", forKey: "Authorization")
        }
